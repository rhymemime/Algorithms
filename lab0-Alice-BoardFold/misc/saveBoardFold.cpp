...
int main(int argc, char **argv){
	...
	if(argc!=2){
		cout << "usage: Board x.cli < boardStuff\n";
		return 1;
	}
	...
	int c = atoi(column.c_str());

	cout << r << " " << c << endl;

	string tmp;
	int cinCounter=0;
	vector<string>compressedGrid;
	while(cin>>tmp){
		compressedGrid.resize(compressedGrid.size()+1);
		compressedGrid[cinCounter] = tmp;
		/*for(int i=0;i<tmp.size();i++){
			compressedGrid[cinCounter].push_back(tmp);
		}*/
		cinCounter++;
	}
	...
	Board b;
	cout << b.howMany(r, c, compressedGrid) << endl;
}
